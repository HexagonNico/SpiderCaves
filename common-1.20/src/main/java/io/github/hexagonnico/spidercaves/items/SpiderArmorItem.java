...
public class SpiderArmorItem extends ArmorItem {
    ...
    public static boolean isWearingFullSet(LivingEntity entity) {
        return entity.getItemBySlot(EquipmentSlot.HEAD).is(RegistryManager.SPIDER_HELMET.get())
            && entity.getItemBySlot(EquipmentSlot.CHEST).is(RegistryManager.SPIDER_CHESTPLATE.get())
            && entity.getItemBySlot(EquipmentSlot.LEGS).is(RegistryManager.SPIDER_LEGGINGS.get())
            && entity.getItemBySlot(EquipmentSlot.FEET).is(RegistryManager.SPIDER_BOOTS.get());
    }
}
